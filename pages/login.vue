<template>
  <main class="flex flex-col items-center justify-center py-16 h-96 gap-4 px-4">
    ...
    >
      ...
      <div class="flex items-center justify-between">
        <NuxtLink class="text-white" to="/forgot-password"> 忘记密码 </NuxtLink>
        <NuxtLink class="text-white" to="/login-with-magic-link">
          通过邮箱链接登录
        </NuxtLink>
      </div>
    </form>
      ...
      if (apiErrors) {
        ...
        if (user.isAdmin) {
          if (this.$route.query && this.$route.query.to) {
            await this.$router.push(this.$route.query.to)
          } else {
